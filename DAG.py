...
query_refresh_dimensions = f"""
Use at3.raw;
ALTER EXTERNAL TABLE raw.raww REFRESH;
...
refresh_raw_tables = SnowflakeOperator(
    task_id='refresh_raw_tables_task',
    sql=query_refresh_raw,
    snowflake_conn_id=snowflake_conn_id,
    dag=dag
)

refresh_dimensions = SnowflakeOperator(
    task_id='refresh_dimensions_task',
    sql=query_refresh_dimensions,
    snowflake_conn_id=snowflake_conn_id,
    dag=dag
)

refresh_facts = SnowflakeOperator(
    task_id='refresh_facts_task',
    sql=query_refresh_facts,
    snowflake_conn_id=snowflake_conn_id,
    dag=dag
)

refresh_star_schema = SnowflakeOperator(
    task_id='refresh_star_schema_task',
    sql=query_refresh_star_schema,
    snowflake_conn_id=snowflake_conn_id,
    dag=dag
)
...
refresh_raw_tables >> refresh_dimensions >> refresh_facts >> refresh_star_schema >> refresh_datamart